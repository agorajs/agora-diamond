(function (global, factory) {
  ...
  (global = global || self, factory(global.AgoraDiamond = {}));
  ...
  */
  ...
  var diamondGraphRotation = function diamondGraphRotation(graph) {
    ...
    var rotatedNodes = lodash.map(graph.nodes, function (n) {
      var polar = toPolar(n);
      polar.theta += Math.PI / 4;
      var cart = toCartesian(polar);
      return _objectSpread2(_objectSpread2({}, n), cart);
    });

    var vs = lodash.sortBy(rotatedNodes, 'x');

    var hs = lodash.sortBy(rotatedNodes, 'y');

    var diamonds = lodash.map(rotatedNodes, function (n) {
      return node2Diamond(n, lodash.findIndex(vs, ['index', n.index]), // i'm sure it exists
      lodash.findIndex(hs, ['index', n.index]) // i'm sure it exists
      );
    });

    var constraints = []; // set minimize constraint

    var minimize = lodash(diamonds).map(function (_ref) {
    ...
    var tmodel = main$2.ReformatLP(lpsolve); // console.log(lpsolve);

    var solver = main$2.Solve(tmodel);

    var feasible = solver.feasible,
        result = solver.result,
        bounded = solver.bounded,
        rest = _objectWithoutProperties(solver, ["feasible", "result", "bounded"]); // index => {x?: y:?}


    var positions = lodash.transform(rest, function (result, val, key) {
    ...
    lodash.forEach(diamonds, function (_ref2) {
    ...
    var updatedNodes = lodash.map(graph.nodes, function (_ref3) {
      var index = _ref3.index,
          x = _ref3.x,
          y = _ref3.y,
          rest = _objectWithoutProperties(_ref3, ["index", "x", "y"]);
